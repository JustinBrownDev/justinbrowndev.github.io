...
const infra=a.placements.filter(p=>p.domain==='facade-infrastructure' || p.domain==='facade-macro');
...
assert.ok(macro.length >= 5, `expected at least one readable multi-part facade assembly, got ${macro.length} primitives`);
assert.ok(a.stats.facadeMacroAssemblies >= 1, `expected facade macro assemblies, got ${a.stats.facadeMacroAssemblies}`);
assert.ok(new Set(macro.map(p=>p.assemblyId)).size === a.stats.facadeMacroAssemblies, 'macro assembly accounting must match emitted groups');
assert.ok(macro.some(p=>Math.max(p.sx,p.sy,p.sz) >= 2), 'macro pass must produce building-scale members, not more micro clutter');
...
assert.ok(a.stats.macroPrimitives === macro.length, 'facade-only fixture should account every macro primitive');
