...
function assertField(label, physics, expectedMinimum) {
  ...
  assert.ok(plan.stats.sceneParts >= plan.places.length, `${label}: each authored place must retain a visible scene payload`);
  assert.ok(plan.stats.scenePaintParts >= plan.places.length, `${label}: each scene needs a matte painted identity cue`);
  ...
  for (const place of plan.places) {
    ...
    assert.ok(place.parts.length > 0, `${label}:${place.id}: scene must retain its identity payload after integration`);
    assert.ok(place.parts.some(part => part.renderClass === 'paint'), `${label}:${place.id}: a painted identity cue must stay on the matte render lane`);
