...
ok(!main.includes('createMusicPlayer') && !main.includes('#musicPlayer'), 'retired music player must not remain wired into runtime UI');
ok(!fs.existsSync(path.join(root, 'systems/music-player.js')), 'retired music-player module should be deleted');
...
console.log('[progressive-runtime] PASS: bounded authored work + self-refining infinite chunks + structural-first rendering + music player retired');
