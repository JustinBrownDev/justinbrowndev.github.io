...
for (const place of first.places) {
  ...
  assert.ok(place.parts.length > 0, `${place.id}: authored scene must retain a visible identity payload`);
  assert.ok(place.parts.some(part => part.role === 'paint-pad'), `${place.id}: authored scene must establish a visual floor/paint identity`);
  assert.ok(place.parts.some(part => part.renderClass === 'paint'), `${place.id}: paint identity must use the matte paint lane`);
