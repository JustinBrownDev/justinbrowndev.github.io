...
function analyzeStairRhythm(payload) {
  ...
  return {
    ...
    speciesCounts: countBy(ownership, item => item?.stairSpecies ?? '(legacy/unknown)'),
    builderCounts: countBy(ownership, item => item?.builder ?? '(legacy/unknown)'),
    budgetCounts: countBy(ownership, item => item?.budget ?? '(legacy/unknown)'),
    landingGrammarCounts: countBy(ownership, item => item?.landingGrammar ?? '(legacy/unknown)'),
    ...
    architectureSpecies: countBy(expressions, item => item?.species ?? item?.family ?? '(none)'),
    expressionCoverage: ownership.length ? Math.min(1, expressions.length / ownership.length) : 0,
    ownership: ownership.map(item => ({
      id: item?.id ?? null, layer: item.layer, topology: item?.stairTopology ?? '(metadata missing)',
      topologyVariant: item?.stairTopologyVariant ?? null, returnHandedness: item?.returnHandedness ?? null, stairAxis: item?.stairAxis ?? null,
      species: item?.stairSpecies ?? '(legacy/unknown)', purpose: item?.purpose ?? null,
      builder: item?.builder ?? null, budget: item?.budget ?? null, throughput: item?.throughput ?? null,
      landingGrammar: item?.landingGrammar ?? null, endpointGrammar: item?.endpointGrammar ?? null, loadPath: item?.loadPath ?? null,
      floors: finite(item?.floors, 0), buildingPlanId: item?.buildingPlanId ?? null,
      routeClass: item?.routeClass ?? 'local', clearWidth: finite(item?.clearWidth, 0), districtRouteId: item?.districtRouteId ?? null,
    })),
    expressions: expressions.map(item => ({
      id: item?.id ?? null, layer: item.layer, family: item?.family ?? '(none)', species: item?.species ?? item?.family ?? '(none)',
      builder: item?.builder ?? null, budget: item?.budget ?? null, loadPath: item?.loadPath ?? null,
      landingGrammar: item?.landingGrammar ?? null, programArchitectureId: item?.programArchitectureId ?? null,
      parts: finite(item?.parts ?? item?.partsPerStory, 0),
    })),
  };
}
...
export function renderStairRhythmSvg(summary, { width = 1500, height = 620 } = {}) {
  ...
  const summaryRows = [
    ...
    `species: ${Object.entries(s.speciesCounts).map(([k,v]) => `${k}×${v}`).join(', ') || 'none'}`,
