export const CAVERN_JOINT_SYNTHESIS_SCHEMA = 'jweb.cavern-joint-synthesis.v1';
...
function freezeDecision(plan, floors, blockers, horizontal) {
...
function pairFloorCaps(ground, ceiling, availableHeight) {
  ...
  // Population parity beats unilateral deletion. Reserve one occupied story for
  // each side, then divide the remaining vertical budget proportionally across
  // both fields' requested extra height.
  let g = gmin;
  let c = cmin;
  ...
  // Rounding leaves small slack. Allocate whole stories to whichever side is
  // furthest below its requested retention ratio; ties alternate by id order.
...
export function reconcileCavernFloorBudgets({
  ...
  for (const g of ground) {
    ...
    for (const c of ceiling) {
      ...
      const caps = pairFloorCaps(g, c, usableHeight);
  ...
  const groundDecisions = new Map(ground.map(plan => [String(plan.id), freezeDecision(plan, gFloors.get(String(plan.id)), gBlockers.get(String(plan.id)), plan.horizontal)]));
  const ceilingDecisions = new Map(ceiling.map(plan => [String(plan.id), freezeDecision(plan, cFloors.get(String(plan.id)), cBlockers.get(String(plan.id)), plan.horizontal)]));
  return Object.freeze({
