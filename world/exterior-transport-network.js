...
export const EXTERIOR_TRANSPORT_NETWORK_SCHEMA = 'jweb.exterior-transport-network.v2';
const EPS = 1e-6;
...
function candidateBlocked(candidate, blocked) {
  ...
  if (candidate.kind === 'walkway-link') {
...
function candidateEnvelope(candidate) {
  ...
  if (candidate.kind === 'walkway-link') {
    return { rect: pathRect(candidate.axis, candidate.aEdge, candidate.bEdge, candidate.fixedCoord, candidate.halfWidth), y0: candidate.y0, y1: candidate.y1 };
  }
  if (candidate.kind === 'stair-link') {
...
function linksConflict(a, b) {
  ...
  const pointsFor = link => link.kind === 'walkway-link'
